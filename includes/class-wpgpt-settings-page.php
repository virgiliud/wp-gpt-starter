 ...
 * The settings page for the WPGPT plugin.
 */
class WPGPT_Settings_Page {
	...
	public static function render_settings_field( $args ) {
		...
		switch ( $args['id'] ) {
			case 'wpgpt_api_key':
				$html .= '<input type="password" id="' . esc_attr( $args['id'] ) . '" name="wpgpt_settings[' . esc_attr( $args['id'] ) . ']" value="' . $value . '" class="regular-text">';
            	$html .= '<p class="description">' . __( 'Enter your OpenAI API key.', 'wpgpt' ) . '</p>';
				break;
			case 'wpgpt_model':
